...
export const ClassTable: React.FunctionComponent<ClassTableProps> = ({
  ...
  const tableRows = Object.keys(metricsByClassByMetric).map((classId) => ({ classId }));
  ...
  const columns: Column<{ classId: string }>[] = [
    ...
    },
    {
      Header: percColText,
      accessor: (row) => {
        const value = metricsByClassByMetric[row.classId][metricIdName][0].value;
    ...
    },
    {
      ...
      accessor: (row, rowIndex) => {
        const value = metricsByClassByMetric[row.classId][percMetricIdName][0].value
        // const valueDisplay = formatPerc
        //   ? percentWithEdge(value)
        //   : value;
        // @ts-ignore: need to add objective to type
        const goal = dataGroup.objective ? formatPerc ? dataGroup.objective.target * 100 : dataGroup.objective.target : 0;
